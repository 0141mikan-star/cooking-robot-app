...
def parse_user_intent(user_message):
    ...
    next_keywords = ["次", "つぎ", "つぎは", "なんする", "何する", "できた", "終わった", "おわった", "おk", "オッケー", "おっけー", "進めて", "進む", "完了", "next", "いいよ"]
    if any(k in msg for k in next_keywords):
        return "NEXT_STEP"
        
    # 2. 聞き返しの意図の解釈
    repeat_keywords = ["もう一回", "もう一度", "聞き取れ", "え？", "なんて", "リピート", "聞こえなかった", "もう一度言って"]
    if any(k in msg for k in repeat_keywords):
        return "REPEAT_STEP"
        
    # 3. 開始・スタートの意図の解釈
    start_keywords = ["スタート", "開始", "はじめ", "やろう", "作ろう"]
    if any(k in msg for k in start_keywords):
        return "START_COOKING"
        
    # 4. リセットの意図の解釈
    reset_keywords = ["リセット", "最初から", "やり直し"]
...
def generate_intelligent_recipes(ingredients_list):
    ...
    if has_egg:
        ...
        r1_steps = [
            ...
            f"ステップ3：最後に、最初に取り出した半熟卵をフライパンに戻し、左に表示されている最適化調味料を入れてサッと全体を合わせたら完成だよ！卵が固くなりすぎないのがコツ！"
        ]
    else:
        r1_steps = [
            f"ステップ1：フライパンに油をひいて、まずは火の通りにくい【{main_item}】を中火で色が変わるまで炒めよう！終わったら教えてね。",
            f"ステップ2：次に残りの具材を投入して、全体に火が通るまで炒めるよ。できたら次！",
    ...
    if has_egg:
        ...
        r2_steps = [
            ...
            f"ステップ3：ここがポイント！火を止める直前に、溶き卵をスープの表面に円を描くように細く回し入れよう。卵がふわっと浮いてきたら完成！これなら固くならないよ！"
        ]
    else:
        r2_steps = [
    ...
    recipes[r2_name] = {
        "condiments": {"お水": "300.0", "鶏ガラスープの素": "1.5", "醤油": "0.5", "ごま油": "0.5", "片栗粉": "1.0"},
        "steps": r2_steps
    }
...
with col_left:
    ...
    st.session_state['base_ingredients'] = edited_ingredients
    ...
    if st.button("🔍 【手順B】この食材から料理を複数提案してもらう", type="secondary", use_container_width=True):
        ...
        if not ingredients_list:
            ...
            proposal_msg = "もっち、食材を解析したよ！卵などの火の通りやすさも計算に入れてレシピを組み立てたからね。下のリストから選んで！"
    ...
    if st.button("✨ 【手順D】選んだ料理の調味料比率を計算する", type="primary", use_container_width=True):
        if not selected_recipe:
            ...
            init_msg = f"【{selected_recipe}】の最適化が完了したよ！右側のハンズフリーを起動して、いつもの話し言葉で『スタート』って話しかけてね！"
